    ...
    <div class="WP-comment-placeholder">
      ...
      <div id="comment-mid">
      	<?php if($this->session->userdata('is_logged_in') == true) {?>
            ...
            <div style="display:none" id="ajax-loader-bar"><img style="margin-left:164px" src="/public/images/ajax-bar.gif" /></div>
            <?php echo form_close(); ?>
        <?php }?>
		...
		?>
          <table width="550px" id="comment_<?=$comment['comment_id']?>" class="<?php if($this->session->userdata('username')==strtolower($comment['username']))echo 'my-comment';?>">
            <tr>
              <td style="vertical-align:top;" width="50"><div style="float: left; width: 180px;"><img style="float: left;padding-right:5px;" src="<?php echo ($comment['avatar'])? '/public/'.$comment['avatar'] : 'http://www.gravatar.com/avatar/'.md5( strtolower( trim( $comment['email'] ) ) );?>" /><p id="commentator"><?php echo $comment['username']; ?></p>
                ...
                <?php endif;?>
                <p id="date-comment">Posted <?php echo date('j M Y', strtotime($comment['comment_created'])); ?> <?php if($myComment || $admin){?>(<a title="Remove this comment" id="remove_<?=$comment['comment_id']?>" class="remove_comment" href="javascript: void(0)"><img alt="Remove Comment" src="/public/images/icon_delete.png"  /></a>)<?php }?></p></td>
            </tr>
          </table>
        <?php } ?>
        </div>
	...
	*/
    ...
    channel.bind('new_comment_<?=$work_data['work_id']?>', function(data) {
		...
		if(data.story_id == '<?=$work_data['work_id']?>'){//new comment on thi story?
		  ...
		  var template = '<table style="display:none" id="comment_'+data.comment_id+'" width="550px" class="'+myclass+'">\
				<tbody><tr>\
			...
			if(myclass=="my-comment")template += ' (<a class="remove_comment" id="remove_'+data.comment_id+'" title="Remove this comment" href="javascript: void(0)"><img alt="Remove Comment" src="/public/images/icon_delete.png"></a>)';
