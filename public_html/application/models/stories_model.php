...
//			"bid_deadline" => $this->input->post('biddead_year').'-'.$this->input->post('biddead_month').'-'.$this->input->post('biddead_day')
 ...
 FROM works, users WHERE users.user_id = works.creator AND works.status in ('open','Open', 'Reject') ORDER BY works.priority DESC, works.work_id ASC";
	...
	function browse_stories_v5($subject, $project_sel, $cat_sel, $type, $skill_sel, $cash_from, $cash_to, $point, $search, $timeS, $timeE, $order){
		//$sql = "SELECT cworks.*, project.project_name FROM (select works.*, users.username from works left join users on works.work_horse=users.user_id) as cworks,project WHERE project.project_id=cworks.project_id ";
	...
	function list_stories_map($percision,$lat,$lng){
		$sql = "SELECT works.work_id, mission_category.category, works.title, works.description, (SELECT count(1) FROM bids WHERE bids.work_id = works.work_id) AS num_bids, (SELECT count(1) FROM comments WHERE story_id = works.work_id) AS num_comments, (SELECT concat(duration,' ',type) FROM arrangement_type as t1, arrangement_time as t2 WHERE t2.type_id=t1.id AND t2.time_id=works.est_time_frame) AS end, works.cost
