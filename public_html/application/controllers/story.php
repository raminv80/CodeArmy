...
class Story extends CI_Controller {
	...
	function setbid()
	{
		...
		if($this->input->post('work_id') && ($data[0]['status']=='open' || $data[0]['status']=='Reject')) {
			...
			$short_message = '<p>User <a href="http://'.$_SERVER['HTTP_HOST'].'/user/'.$this->session->userdata('user_id').'">'.$this->session->userdata('username').'</a> placed a bid on story <a href="http://'.$_SERVER['HTTP_HOST'].'/story/'.$work_data[0]['work_id'].'">'.$work_data[0]['title'].'</a> with amount '.$this->input->post('set_cost').' and days '.$this->input->post('set_days').'</p>';
			$this->notify(noreply_email,email_name.' Bids', $product_owner, admin_cc, $title,$message, 'bid', $product_owner_id, $short_message);
			...
			if(($this->input->post('set_cost') <  $lowest['bid_cost'] && $this->input->post('set_days') <  $lowest['days']) ||
			   ...
			   ($this->input->post('set_cost') == $lowest['bid_cost'] && $this->input->post('set_days') <  $lowest['days'])) {
				...
				if ($lowest['user_id'] != $user_id) {
					...
					$short_message = '<p>You were outbided on story <a href="http://'.$_SERVER['HTTP_HOST'].'/story/'.$work_data[0]['work_id'].'">'.$work_data[0]['title'].'</a>.</p>';
					$this->users->notify($lowest['user_id'], $title, $message, 'bid',NULL, $short_message);
				}
			};
		}
	...
	function comments() {	
		...
		$short_message = "<p>Cooment on <a href='http://".$_SERVER['HTTP_HOST']."/story/".$this->input->post('story_id')."'>".$work[0]['title']."</a> by <a href='http://".$_SERVER['HTTP_HOST']."/user/".$this->session->userdata('user_id')."'>".$this->session->userdata('username')."</a>: \"".substr(strip_tags($this->input->post('comments')),0,500)."\"</p>";
		$this->notify(noreply_email,email_name, $to, $cc, $subject, $message, 'message', $to_id, $short_message);
	...
	function reopen($work_id){ 
	   $this->check_authentication();
		...
		if($this->projects_model->is_project_owner($user_id, $work_data[0]['project_id'])){
			...
			$short_message = '<p>Seems product owner is not satisfied on assigning story <a href="http://'.$_SERVER['HTTP_HOST'].'/story/'.$story_id.'">'.$work_data[0]['title'].'</a> to you. Regard to this matter, product owner has decided to revoke your assignment and the job is set to open for bidding.</p>';
			...
			$short_message = '<p>The story <a href="http://'.$_SERVER['HTTP_HOST'].'/story/'.$story_id.'">'.$work_data[0]['title'].'</a> is not proceeding according to product owner expectations. Regard to this matter, product owner has decided to revoke the assignment and the job is set to open for bidding one again.</p>';
    ...
    function bid_accept($id, $ref='') {
			   ...
			   if($this->projects_model->is_project_owner($user_id, $project_id)){
				   ...
				   $this->notify(noreply_email,email_name, $to, admin_cc, $title,$message, 'bid', $to_id, $short_message);
				   $data = array(
					"user_id" => $to_id,
					"title" => "Assignment",
...
//				$this->view_data['formdate_biddead'] = $formdate_biddead;
		...
		function verify($id){
			...
			if($this->projects_model->is_scrum_master($user_id, $work_data[0]['project_id'])){
				...
				$short_message = '<p>Story <a href="http://'.$_SERVER['HTTP_HOST'].'/story/'.$story_id.'">'.$work_data[0]['title'].'</a> is verified by scrum master. Now this job is awaiting for approval of product owner.</p>';
				//$this->notify(admin_email,email_name, $to, admin_cc, $title,$message);
				$this->users_model->notify($user_data[0]['user_id'], $title, $message, 'job', NULL, $short_message);
		...
		function signoff($id){
			...
			if($this->projects_model->is_project_owner($user_id, $work_data[0]['project_id'])){
				...
				$this->notify(noreply_email,email_name, $to, admin_cc, $title,$message, 'job', $to_id, $short_message);
		...
		private function notify($from,$fromName, $to, $cc, $subject, $message, $category=NULL, $to_id = NULL, $shor_message = ""){
